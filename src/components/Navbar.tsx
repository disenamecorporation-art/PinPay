...
interface NavbarProps {
  ...
  onOpenTransfer: () => void;
  activeTab: 'home' | 'calculator' | 'admin' | 'dashboard' | 'vision';
  onSelectTab: (tab: 'home' | 'calculator' | 'admin' | 'dashboard' | 'vision') => void;
  currentUser?: { name: string; email: string, role?: 'admin' | 'user' } | null;
  onLogout?: () => void;
}

export const Navbar: React.FC<NavbarProps> = ({ onOpenAuth, onOpenTransfer, activeTab, onSelectTab, currentUser, onLogout }) => {
    ...
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200/80 text-slate-800 shadow-xs transition-all">
        ...
        <div className="flex items-center justify-between h-20">
          ...
          <div className="hidden md:flex items-center space-x-6">
            ...
            <button 
              ...
              className={`text-sm font-semibold transition-colors px-3 py-1.5 rounded-xl border ${activeTab === 'vision' ? 'bg-[#f43a8e] text-white border-[#f43a8e]' : 'border-slate-200 text-slate-700 bg-slate-50 hover:bg-slate-100'}`}
            >
              Tecnología & Visión
            </button>
      ...
      {isOpen && (
        <div className="md:hidden bg-white border-b border-slate-200 px-4 pt-3 pb-6 space-y-3 shadow-lg">
          ...
          <button 
            onClick={() => { setIsOpen(false); onSelectTab('vision'); }}
            className="w-full text-left px-3 py-2 rounded-lg text-base font-semibold text-white bg-[#f43a8e]"
          >
            Tecnología & Visión
          </button>
          <a 
